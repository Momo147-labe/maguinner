...
class ReportsContent extends StatefulWidget {
  ...
  const ReportsContent({Key? key, required this.currentUser}) : super(key: key);
...
class _ReportsContentState extends State<ReportsContent>
    ...
    for (final sale in _sales) {
      if (sale.id != null) {
        ...
        for (final line in saleLines) {
          ...
          if (product != null) {
            final profit =
                (line.salePrice - (product.purchasePrice ?? 0)) * line.quantity;
            totalProfit += profit;
          }
        }
      }
  ...
  Widget _buildHeader() {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          if (isMobile) _buildMobileHeader(isDark) else _buildDesktopHeader(isDark),
  ...
  Widget _buildSalesReport() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildSalesCards(isMobile, totalSales),
          SizedBox(height: isMobile ? 16 : 24),
          _buildSalesTable(isDark, isMobile),
        ],
      ),
    );
  }

  Widget _buildSalesCards(bool isMobile, double totalSales) {
    if (isMobile) {
      return Column(
  ...
  Widget _buildSalesTable(bool isDark, bool isMobile) {
    return Card(
                    ...
                    .map(
                      (sale) => DataRow(
                        cells: [
                          ...
                          DataCell(
                            Text(_userNames[sale.userId] ?? 'Inconnu'),
                          ),
                          DataCell(
                            Text(
  ...
  Widget _buildPurchasesReport() {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    double totalPurchases = _purchases.fold(
      0,
                          ...
                          .map(
                            (purchase) => DataRow(
                              cells: [
                                ...
                                DataCell(
                                  Text(purchase.paymentType ?? 'direct'),
                                ),
                                DataCell(
                                  IconButton(
                                    icon: const Icon(Icons.visibility),
  ...
  Widget _buildDetailedUserReportCard(User user) {
    ...
    final totalUserSales = userSales.fold(
      0.0,
      (sum, sale) => sum + (sale.totalAmount ?? 0),
    );
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      _buildDetailRow(
                        'Client',
                        _customerNames[sale.customerId] ?? 'Client Occasionnel',
                      ),
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Text(
                            '${line.quantity} x ${CurrencyFormatter.formatGNF(line.salePrice)}',
                          ),
                          trailing: Text(
                            CurrencyFormatter.formatGNF(line.subtotal),
  ...
  Future<void> _showPurchaseDetails(Purchase purchase) async {
    final purchaseLines = await DatabaseHelper.instance.getPurchaseLines(
      purchase.id!,
    );
    if (!mounted) return;
            ...
            children: [
              ...purchaseLines.map(
                (line) => ListTile(
                  ...
                  subtitle: Text(
                    'Quantité: ${line.quantity} × ${line.purchasePrice} GNF',
                  ),
            ...
            children: [
              ...
              pw.Table.fromTextArray(
                ...
                data: _users.map((user) {
                  final userSales = _sales
                      .where((sale) => sale.userId == user.id)
                      .toList();
                  final totalUserSales = userSales.fold(
                    0.0,
                    (sum, sale) => sum + (sale.totalAmount ?? 0),
  ...
  Future<void> _generateUsersExcel() async {
    final result = await FilePicker.platform.saveFile(
      ...
      fileName:
          'rapport_utilisateurs_${DateTime.now().millisecondsSinceEpoch}.csv',
    );
